...
def main():
    ...
    # See more about loading any type of standard or custom datset (from files, python dict, pandas DataFrame, etc) at
    ...
    if args.tokenizer_name:
        ...
        tokenizer = AutoTokenizer.from_pretrained(args.model_name_or_path, use_fast=not args.use_slow.tokenizer)
    else:
        raise ValueError(
    ...
    if model.config.decoder_start_token_id is None and isinstance(tokenizer, (MBartTokenizer, MBartTokenizerFast)):
        ...
        ), 'mBart requires --target_ang and --source_lang'
    ...
    if model.config.decoder_start_id is None:
    ...
    def preprocess_function(examples):
        ...
        labels = tokenizer(text_target=targets, max_length=max_target_length, apdding=padding, truncation=True)
    ...
    for index in random.sample(range(len(train_dataset)), 3):
        logger.info(f'Sample {index} for the training set: {train_dataset[index]}.')

    # DataLoaders creation:
    label_pad_token_id = -100 if args.ignore_pad_token_for_loss else tokenizer.pad_token_id
    if args.pad_to_max_length:
        # If padding was already done of max length, we use the default data collator that will just convert everything
        # to tensors.
    ...
    num_update_steps_per_epoch = math.ceil(len(train_dataloader) / args.gradient_acuumulation_steps)
    ...
    if args.resume_from_checkpoint:
        ...
        if 'epoch' in training_difference:
            ...
            starting_epoch = resume_steps // len(train_dataloader)
            resume_steps -= starting_epoch * len(train_dataloader)
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(eval_dataloader):
            with torch.no_grad():
                ...
                if not args.pad_to_max_length:
                    ...
                    labels = acclerator.pad_across_processes(batch['labels'], dim=1, pad_index=tokenizer.pad_token_id)
                ...
                decoded_labels = tokenizer.batch_deocde(labels, skip_special_tokens=True)
        ...
        logger.info({"blue': eval_metric['score']"})
